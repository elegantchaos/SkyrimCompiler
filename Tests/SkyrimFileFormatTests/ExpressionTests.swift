...
class ExpressionBuildingTests: XCTestCase {
    ...
    func testGetStage() {
        let expression = ConditionExpression(from: RawExpression(function: 58, value: 1092616192, comparison: .equals, flags: [], parameters: [219994, 0], runOn: .subject))
        XCTAssertEqual(expression.rawValue, "Subject.GetStage(0x035B5A) == 10.0")
    }
    
    func testGetIsAliasRef() {
        let expression = ConditionExpression(from: RawExpression(function: 566, value: 1065353216, comparison: .equals, flags: [], parameters: [1, 0], runOn: .subject))
        XCTAssertEqual(expression.rawValue, "Subject.GetIsAliasRef(0x1) == 1.0")
    }
     
    func testIsScenePlaying() {
        let expression = ConditionExpression(from: RawExpression(function: 248, value: 0, comparison: .equals, flags: [], parameters: [220000, 0], runOn: .subject))
        XCTAssertEqual(expression.rawValue, "Subject.IsScenePlaying(0x035B60) == 0.0")
    }
    
    func testGetBribeSuccess() {
        let expression = ConditionExpression(from: RawExpression(function: 654, value: 1065353216, comparison: .equals, flags: [], parameters: [0, 0], runOn: .subject))
        XCTAssertEqual(expression.rawValue, "Subject.GetBribeSuccess() == 1.0")
    }
    
    func testGetActorValue() {
        let expression = ConditionExpression(from: RawExpression(function: 14, value: 857763, comparison: .greaterThanOrEqual, flags: [.useGlobal], parameters: [17, 0], runOn: .subject))
